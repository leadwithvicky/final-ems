      ...
      <div className="space-y-6">
        ...
        <div className="card">
          ...
          <div className="p-6">
            ...
            {selectedTab === 'payroll' && (
              <div className="space-y-6">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Payroll History</h3>
                <div className="space-y-3">
                  {payroll.map((record: any, index: number) => (
                    <div key={record._id || index} className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-medium text-gray-900 dark:text-gray-100">
                          {record.month}/{record.year}
                        </h4>
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                          record.status === 'paid' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-200' :
                          record.status === 'processed' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-200' :
                          'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200'
                        }`}>
                          {record.status}
                        </span>
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          <p className="text-gray-600 dark:text-gray-400">Basic Salary: ${record.basicSalary}</p>
                          <p className="text-gray-600 dark:text-gray-400">Allowances: ${record.totalEarnings - record.basicSalary}</p>
                          <p className="text-gray-600 dark:text-gray-400">Deductions: ${record.totalDeductions}</p>
                        </div>
                        <div className="text-right">
                          <p className="font-semibold text-gray-900 dark:text-gray-100">Net Salary: ${record.netSalary}</p>
                          {record.paymentDate && (
